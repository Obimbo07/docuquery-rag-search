 ...
 */
import { download as api_documents_download_download } from "~backend/documents/download";
import { generate as api_documents_generate_generate } from "~backend/documents/generate";
import { list as api_documents_list_list } from "~backend/documents/list";
import { search as api_documents_search_search } from "~backend/documents/search";
import { upload as api_documents_upload_upload } from "~backend/documents/upload";
...
export namespace documents {
    ...
    export class ServiceClient {
         ...
         */
        public async download(params: { id: string }): Promise<void> {
            await this.baseClient.callTypedAPI(`/documents/${encodeURIComponent(params.id)}/download`, {method: "GET", body: undefined})
        }

        /**
         * Generates an AI-powered answer based on the query and retrieved context.
         */
        public async generate(params: RequestType<typeof api_documents_generate_generate>): Promise<ResponseType<typeof api_documents_generate_generate>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/generate`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_documents_generate_generate>
        }

        /**
         * Lists all uploaded documents.
         */
        public async list(): Promise<ResponseType<typeof api_documents_list_list>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/documents`, {method: "GET", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_documents_list_list>
        }

        /**
         * Searches documents using semantic vector similarity.
         */
        public async search(params: RequestType<typeof api_documents_search_search>): Promise<ResponseType<typeof api_documents_search_search>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/search`, {method: "POST", body: JSON.stringify(params)})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_documents_search_search>
        }

        /**
         * Uploads a PDF document and processes it for search.
         */
        public async upload(): Promise<ResponseType<typeof api_documents_upload_upload>> {
            // Now make the actual call to the API
            const resp = await this.baseClient.callTypedAPI(`/upload`, {method: "POST", body: undefined})
            return JSON.parse(await resp.text(), dateReviver) as ResponseType<typeof api_documents_upload_upload>
        }
    }
}
